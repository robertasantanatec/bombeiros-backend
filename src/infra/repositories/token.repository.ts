import {
  Injectable,
  HttpException,
  HttpStatus,
  Inject,
  forwardRef,
} from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { Token } from 'src/infra/entities/token.entity';
import { User } from 'src/infra/entities/user.entity';
import { UserService } from 'src/infra/services/user.service';
import type { TTokenRepository } from 'src/domain/repositories/itoken.repository';

@Injectable()
export class TokenRepository implements TTokenRepository {
  constructor(
    @InjectRepository(Token)
    private readonly tokenRepository: Repository<Token>,
    private readonly _userService: UserService,
  ) {}

  async save(hash: string, username: string) {
    const objToken = await this.tokenRepository.findOne({
      where: { username: username },
    });
    if (objToken) {
      await this.tokenRepository.update(objToken.id, {
        hash: hash,
      });
    } else {
      await this.tokenRepository.insert({
        hash: hash,
        username: username,
      });
    }
  }

  async update(id: string, hash: string): Promise<void> {
    await this.tokenRepository.update(id, { hash: hash });
  }

  // agora apenas retorna o usuário associado ao token ou lança erro
  async refreshToken(oldToken: string): Promise<User> {
    const objToken = await this.tokenRepository.findOne({
      where: { hash: oldToken },
    });
    if (objToken) {
      const user = await this._userService.findOne(objToken.username);
      if (!user) {
        throw new HttpException({ errorMessage: 'Usuário não encontrado' }, HttpStatus.UNAUTHORIZED);
      }
      return user as unknown as User;
    } else {
      throw new HttpException({ errorMessage: 'Token inválido' }, HttpStatus.UNAUTHORIZED);
    }
  }

  async getUserByToken(token: string): Promise<User | null> {
    token = token.replace('Bearer ', '').trim();
    const objToken: Token | null = await this.tokenRepository.findOne({
      where: { hash: token },
    });
    if (objToken) {
      const user = await this._userService.findOne(objToken.username);
      return user as unknown as User;
    } else {
      return null;
    }
  }

  async findOneByHash(hash: string): Promise<Token | null> {
    return await this.tokenRepository.findOne({
      where: { hash: hash },
    });
  }

  async findOneByUsername(username: string): Promise<Token | null> {
    return await this.tokenRepository.findOne({
      where: { username: username },
    });
  }
}
